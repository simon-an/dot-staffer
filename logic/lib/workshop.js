...
const vote_transformer_1 = require("./vote-transformer");
...
function validateInput(input) {
    ...
    const notVoted = Object.values(input.sessionHolders).reduce((sum, current) => {
    ...
    input.votes.forEach(vote => {
        const name = helpers_1.isVote(vote) ? vote.person : vote;
...
function simpleMagic(input) {
    const persons = vote_transformer_1.toMapTopicSelectedByPerson(input.votes);
    const personvotes = vote_transformer_1.toMapPrioByPersonAndTopic(input.votes, input.topics.map(t => t.id));
    const votesUsed = [];
    ...
    sessions.forEach(session => {
        if (session.discussers.length < input.config.discussers) {
            ...
            const candidates = vote_transformer_1.filterAndSortCandidatesByTopicId(personvotes, persons, session.topicId);
...
exports.simpleMagic = simpleMagic;
function magic(input) {
    ...
    let i = 0;
    const personBusy = {};
    const slottedTopics = input.topics.reduce((result, topic) => {
        const slot = helpers_1.calculateSlot(i, topicsPerSlot);
        i++;
    ...
    helpers_1.assignDiscusser(1, sessions, input, personBusy);
    helpers_1.assignDiscusser(2, sessions, input, personBusy);
    helpers_1.assignDiscusser(3, sessions, input, personBusy);
